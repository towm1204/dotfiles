...
- **CRITICAL — Code review**: ANY request related to reviewing code — including "review", "diff", "check changes", "look at changes", "review this branch", "review PR", or any variant — MUST immediately invoke the `code-reviewer` subagent via the Agent tool. Never run `git diff` yourself. Never do an inline review. Do not do any preliminary work — invoke the agent as the first and only action. When the agent returns, relay its full output verbatim — do NOT summarize or condense it.
